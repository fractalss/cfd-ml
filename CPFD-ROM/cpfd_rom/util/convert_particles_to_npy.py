...
def convert_ascii_to_npy(input_dir, output_dir, field_variable):
    ...
    for file in tqdm(files, desc=f"Converting {input_dir}"):
        ...
        if not all(col in df.columns for col in ['x', 'y', 'z', 'Cloud Id', field_variable]):
            print(f"[WARNING] Skipping {file}: required columns missing")
            continue

        array_data = df[['x', 'y', 'z', 'Cloud Id', field_variable]].values.astype(np.float32)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input_dir", required=True, help="Input directory containing particles*.txt files")
